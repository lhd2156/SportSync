/**
 * SportSync - Google Auth Button
 *
 * Styled Google OAuth button following Google branding guidelines.
 * White background, Google G icon, proper spacing.
 */
...
function GoogleAuthButton({ onClick, label = "Continue with Google", disabled }: GoogleAuthButtonProps) {
  return (
    <button
      type="button"
      onClick={onClick}
      disabled={disabled}
      className="surface-google-auth-button w-full py-3 font-medium rounded-lg flex items-center justify-center gap-3 transition-all disabled:opacity-40 disabled:cursor-not-allowed"
    >
      <svg className="w-5 h-5" viewBox="0 0 24 24">
        <path fill="var(--google-blue)" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z" />
